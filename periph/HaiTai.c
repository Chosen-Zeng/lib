...
#ifdef CAN_SUPPORT
    CAN_SendData(CAN_handle, CAN_ID_STD, HAITAI_cmd << 4 | HaiTai[idx].ID, HaiTai[idx].TxData, len);
#elif defined FDCAN_SUPPORT
    CAN_SendData(CAN_handle, FDCAN_STANDARD_ID, HAITAI_cmd << 4 | HaiTai[idx].ID, HaiTai[idx].TxData, len);
#endif
}
...
void HaiTai_SendCmd_RS485(USART_handle_t *const UART_info, const unsigned char idx, const unsigned char HAITAI_cmd)
{
    ...
    *(unsigned short *)&HaiTai[idx].TxData[5 + HaiTai[idx].TxData[4]] = CRCsw_Calc(&CRC_16_MODBUS, HaiTai[idx].TxData, 5 + HaiTai[idx].TxData[4]);

    UART_SendArray(UART_info, HaiTai[idx].TxData, 7 + HaiTai[idx].TxData[4]);
}

bool HaiTai_MsgHandler_CAN(const unsigned CAN_ID, const unsigned char idx, const unsigned char RxData[8])
{
    if (CAN_ID == (HAITAI_ENC_INFO << 4 | HaiTai[idx].ID))
    {
        HaiTai[idx].fdbk.pos_sgl = *(unsigned short *)RxData * HAITAI_fPOS;
